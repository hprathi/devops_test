...
namespace Microsoft.Dynamics365.UIAutomation.Api
{
    ...
    public class XrmPage : BrowserPage
    {
        ...
        public bool SwitchToResultsFrame()
        {
            return this.Execute("Switch to Results frame", driver => SwitchToResults());
        }

        internal bool SwitchToResults()
        {
            Browser.Driver.SwitchTo().Frame("resultFrame");
        ...
        public BrowserCommandResult<bool> ClickCommandbyClass(string name, string subName = "", bool moreCommands = false, int thinkTime = Constants.DefaultThinkTime)
        {
            Browser.ThinkTime(thinkTime);

            return this.Execute(GetOptions("Click Command By Class"), driver =>
            {
                driver.FindElement(By.ClassName(name)).Click();
                return true;
            });
        }
        public BrowserCommandResult<bool> ClickCommandbyXPath(string name, string subName = "", bool moreCommands = false, int thinkTime = Constants.DefaultThinkTime)
        {
            Browser.ThinkTime(thinkTime);

            return this.Execute(GetOptions("Click Command By XPath"), driver =>
            {
                driver.FindElement(By.XPath(name)).Click();
                return true;
            });
        }
        public BrowserCommandResult<bool> ClickCommandbyID(string name, string subName = "", bool moreCommands = false, int thinkTime = Constants.DefaultThinkTime)
        {
            Browser.ThinkTime(thinkTime);

            return this.Execute(GetOptions("Click Command By ID"), driver =>
            {
